...
if($mode == 'mergesave') {
	...
	$result =  $adb->pquery("SELECT count(*) AS count FROM {$table_prefix}_crmentity WHERE crmid=? and deleted=0", array($merge_id));
	...
	<script>closePopup(); parent.location.href = parent.location.href;</script>
	<!-- crmv@22022e -->
<?php
